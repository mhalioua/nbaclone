	...
	task :closingline => :environment do
		...
		games.each do |game|
			...
			if date == previous_date
				...
				previous_date = date
				url = "http://www.donbest.com/nba/odds/totals/#{date}.html"
				doc = Nokogiri::HTML(open(url))

				home = Array.new

				doc.css(".oddsTeamWLink").each_with_index do |team, index|
					text = team.text
					space = text.rindex(" ")
					if space == nil # Sometimes teams have capital letters
						next
					end
					text = text[space+1..-1]
					if text == "Blazers" || text == "Trailblazers"
						text = "Trail Blazers"
					end
					if index%2 == 1
				...
				sbg = Array.new
				betus = Array.new
				betphoenix = Array.new
				easystreet = Array.new
				bodog = Array.new
				jazz = Array.new
				sportsbet = Array.new
				bookmaker = Array.new
				...
				# Grab all the closinglines for the past 
				var = 0
				doc.css(".alignRight").each do |stat|
					text = stat.text
				...
				(0..home.size-1).each do |n|
					...
					# out of today's games, what team had a 
					cl_game = todays_games.where(:home_team_id => past_team.id).first
					if cl_game != nil
						cl_game.update_attributes(:opener => opener[n], :westgate => westgate[n], :mirage => mirage[n], :station => station[n], :pinnacle => pinnacle[n],
							:sia => sia[n], :sbg => sbg[n], :betus => betus[n], :betphoenix => betphoenix[n], :easystreet => easystreet[n], :bodog => bodog[n], :jazz => jazz[n],
							:sportsbet => sportsbet[n], :bookmaker => bookmaker[n], :dsi => dsi[n])
						puts cl_game.url
					else
						puts year + month + day
