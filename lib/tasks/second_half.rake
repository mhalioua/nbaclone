	...
	task :total_algorithm => :environment do
		...
		seasons.each do |season|
			season.games.each do |game|
				...
				game.update_attributes(:second_half_ps => predicted)
				puts game.url
				puts predicted
			end
		end
	...
	task :spread_algorithm => :environment do
		...
		seasons.each do |season|
			season.games.each do |game|
				...
				game.update_attributes(:away_second_half_ps => away_predicted, :home_second_half_ps => home_predicted)
				puts game.url
				puts away_predicted
				puts home_predicted
			end
		...
		seasons.each do |season|
			...
			season.game_dates.each do |game_date|
				...
				game_date.games.each do |game|
					...
					ps = game.second_half_ps
					cl = game.second_half_open
					if cl == nil
						cl = game.second_half_cl
					end
					...
					fs = lineups[6].pts + lineups[7].pts + lineups[8].pts + lineups[9].pts
		...
		seasons.each do |season|
			...
			season.game_dates.each do |game_date|
				...
				game_date.games.each do |game|
					...
					away_ps = game.away_second_half_ps
					home_ps = game.home_second_half_ps
					cl = game.second_half_spread
	...
	task :closing_line_totals => :environment do
		...
		Season.where(:year => "2015").each do |season|
			...
			season.games.each do |game|
				...
				url = "http://www.sportsbookreview.com/betting-odds/nba-basketball/totals/2nd-half/?date=#{date}"
				...
				(0...home.size).each do |n|
					...
					if cl_game != nil
						cl_game.update_attributes(:second_half_cl => close[n], :second_half_open => open[n]) # place the first_half cl in the 
						puts cl_game.url
						puts open[n]
						puts close[n]
					else
	...
	task :closing_line_spread => :environment do
		...
		Season.where("id != 1").each do |season|
			...
			season.games.each do |game|
				...
				url = "http://www.sportsbookreview.com/betting-odds/nba-basketball/2nd-half/?date=#{date}"
				doc = Nokogiri::HTML(open(url))
				...
				(0...home.size).each do |n|
					...
					if spread_game != nil
						spread_game.update_attributes(:second_half_spread => line) # place the first_half cl in the 
